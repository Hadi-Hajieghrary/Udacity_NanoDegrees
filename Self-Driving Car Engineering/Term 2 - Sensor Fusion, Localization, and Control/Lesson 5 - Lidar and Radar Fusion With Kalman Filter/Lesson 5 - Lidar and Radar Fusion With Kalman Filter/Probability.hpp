...
public:
    ...
    VectorXd draw() const{
        ...
        srand( (unsigned)time( NULL ) );
        VectorXd Xd = VectorXd::Zero(_mean.rows());
        double n = 100.0;
        for (int i = 0; i<n; i++){
            VectorXd dX = VectorXd::Random(_mean.rows());
            Xd = Xd + 0.5*dX + 0.5* VectorXd::Ones(_mean.rows());
        }
        Xd = Xd - MatrixXd::Ones(_mean.rows(),1)*n/2;
        Xd = Xd/sqrt(n/12.0);
        Eigen::LLT<MatrixXd> lltOfA(_covar); // compute the Cholesky decomposition of A
